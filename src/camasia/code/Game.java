...
public class Game extends Canvas implements Runnable {
	 ...
	 public static final String NAME = "Camasia";
	 // This is the height of the game * scale
	 ...
	 private int[] pixels = ((DataBufferInt) image.getRaster( ).getDataBuffer( ))
				.getData( ); // the array of pixels that will be displayed on the screen.
...
//		  levels[2] = new Level( 128, 128, -1, levels[3] ); // creates the mines (iron level)
	 ...
	 private void init( ) {
		...
		/* This sets up the screens, loads the icons.png spritesheet. */
		  try {
				screen = new Screen( WIDTH, HEIGHT, new SpriteSheet( ImageIO
						  .read( Game.class.getResourceAsStream( "/icons.png" ) ) ) );
				lightScreen = new Screen( WIDTH, HEIGHT, new SpriteSheet( ImageIO
						  .read( Game.class.getResourceAsStream( "/icons.png" ) ) ) );
		  } catch ( IOException e ) {
				e.printStackTrace( );
		  }
	 ...
	 public void run( ) {
		  ...
		  while ( running ) {
				...
				if ( System.currentTimeMillis( ) - lastTimer1 > 1000 ) { //updates every 1 second
					 ...
					 // prints out the number of ticks, and the amount of frames to the console.
	 ...
	 public void tick( ) {
		  ...
		  if ( !hasFocus( ) ) {
				...
				if ( !player.removed && !hasWon )
					 gameTime++; //increases tickCount by 1, this is used for the timer on the death screen.
	 ...
	 public void render( ) {
		  ...
		  if ( xScroll > level.w * 16 - screen.w - 16 )
				xScroll = level.w * 16 - screen.w - 16; // if the screen is at the right border, then stop scrolling.
		  if ( yScroll > level.h * 16 - screen.h - 16 )
				yScroll = level.h * 16 - screen.h - 16; // if the screen is at the bottom border, then stop scrolling.
...
//		  }
	 ...
	 private void renderGui( ) {
		  ...
		  for ( int i = 0; i < 10; i++ ) {
				if ( i < player.health )
					 screen.render( i * 8, screen.h - 16, 0 + 12 * 32, Color
								.get( 000, 200, 500, 533 ), 0 );//renders your current red hearts.
				else
					 screen.render( i * 8, screen.h - 16, 0 + 12 * 32, Color
								.get( 000, 100, 000, 000 ), 0 );//renders black hearts for damaged health.
				
				if ( player.staminaRechargeDelay > 0 ) {
					 if ( player.staminaRechargeDelay / 4 % 2 == 0 )
						  screen.render( i * 8, screen.h - 8, 1 + 12 * 32, Color
									 .get( 000, 555, 000, 000 ), 0 );//creates the blinking effect when you run out of stamina. (white part)
					 else
						  screen.render( i * 8, screen.h - 8, 1 + 12 * 32, Color
									 .get( 000, 110, 000, 000 ), 0 );//creates the blinking effect when you run out of stamina. (gray part)
				} else {
					 if ( i < player.stamina )
						  screen.render( i * 8, screen.h - 8, 1 + 12 * 32, Color
									 .get( 000, 220, 550, 553 ), 0 );//renders your current stamina
					 else
						  screen.render( i * 8, screen.h - 8, 1 + 12 * 32, Color
									 .get( 000, 110, 000, 000 ), 0 );//renders your uncharged stamina (grayed)
				}
		  }
		  if ( player.activeItem != null ) {
				player.activeItem
						  .renderInventory( screen, 10 * 8, screen.h - 16 );//if you have an active item then it will render the item sprite and it's name.
		  }
	 ...
	 private void renderFocusNagger( ) {
		  ...
		  for ( int x = 0; x < w; x++ ) {
				screen.render( xx + x * 8, yy - 8, 1 + 13 * 32, Color
						  .get( -1, 1, 5, 445 ), 0 ); // renders the top part of the box
				screen.render( xx + x * 8, yy + 8, 1 + 13 * 32, Color
						  .get( -1, 1, 5, 445 ), 2 ); // renders the bottom part of the box
		  }
		  for ( int y = 0; y < h; y++ ) {
				screen.render( xx - 8, yy + y * 8, 2 + 13 * 32, Color
						  .get( -1, 1, 5, 445 ), 0 ); // renders the left part of the box
				screen.render( xx + w * 8, yy + y * 8, 2 + 13 * 32, Color
						  .get( -1, 1, 5, 445 ), 1 ); // renders the right part of the box
		  }
		  
		  if ( (tickCount / 20) % 2 == 0 ) {
				Font.draw( msg, screen, xx, yy, Color
						  .get( 5, 333, 333, 333 ) ); //renders the text with a flash effect. (medium yellow color)
		  } else {
				Font.draw( msg, screen, xx, yy, Color
						  .get( 5, 555, 555, 555 ) ); //renders the text with a flash effect. (bright yellow color)
		  }
	 }
	 ...
	 /**
	  * @return The average frames
	  */
	 public int getTotalFramesAverage( ) {
		  return totalFrames / (gameTime / 60);
	 }
	 
	 /**
	  * @return The average ticks
	  */
	 public int getTotalTicksAverage( ) {
		  return tickCount / (gameTime / 60);
	 }
