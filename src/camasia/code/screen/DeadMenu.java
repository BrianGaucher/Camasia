...
public class DeadMenu extends Menu {
	...
	/* WonMenu & DeadMenu are very similar... scratch that, the exact same class with text changes. */
	 ...
	 public void render(Screen screen) {
		  ...
		  Font.draw( timeString, screen, (2 + 5) * 8, 5 * 8, Color
					 .get( -1, 550, 550, 550 ) );// Draws the current time next to "Time:"
		  Font.draw( "Score:", screen, 2 * 8, 6 * 8, Color.get( -1, 555, 555, 555 ) );// Draws "Score:" on the frame
		  Font.draw( "" + game.player.score, screen, (2 + 6) * 8, 6 * 8, Color
					 .get( -1, 550, 550, 550 ) );// Draws the current score next to "Score:"
