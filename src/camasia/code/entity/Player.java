...
import camasia.code.Game;
import camasia.code.InputHandler;
...
public class Player extends Mob {
	 ...
	 private boolean use( ) {
		  ...
		  return dir == 2 && use( x - 12, y - 8 - 2, x - 4, y + 8 - 2 );
	 }
	 ...
	 private void attack( ) {
		  ...
		  if ( activeItem != null ) { // if the player has a active Item
			...
			/* if the interaction between you and an entity is successful then done = true */
				...
				if ( xt >= 0 && yt >= 0 && xt < level.w && yt < level.h ) { // if (xt & yt) are larger or equal to 0 and less than the level's width and height...
					 if ( activeItem.interactOn( level
								.getTile( xt, yt ), level, xt, yt, this, attackDir ) ) { // if the interactOn() method in an item returns true...
						  done = true; // done equals true
					 } else {
						  if ( level.getTile( xt, yt )
										.interact( level, xt, yt, this, activeItem, attackDir ) ) { // if the interact() method in an item returns true...
								done = true; // done equals true
						  }
					 }
		  ...
		  if ( activeItem == null || activeItem
				...
				if ( xt >= 0 && yt >= 0 && xt < level.w && yt < level.h ) { // if (xt & yt) are larger or equal to 0 and less than the level's width and height...
					 level.getTile( xt, yt ).hurt( level, xt, yt, this, random
								.nextInt( 3 ) + 1, attackDir ); // calls the hurt() method in that tile's class
				}
		  }
	 ...
	 public void render(Screen screen) {
		...
		/* where to draw the sprite relative to our position */
		  ...
		  if ( attackTime > 0 && attackDir == 2 ) { // if the attack time is larger than 0 and the attack Direction is 2 (Left)
				...
				screen.render( xo - 4, yo + 8, 7 + 13 * 32, Color
						  .get( -1, 555, 555, 555 ), 3 ); // render a mirrored half-slash below it.
				if ( attackItem != null ) { // if the player has an item
					 attackItem.renderIcon( screen, xo - 4, yo + 4 ); // then render the icon of the item.
				}
		  }
		  if ( attackTime > 0 && attackDir == 3 ) { // if the attack time is larger than 0 and the attack Direction is 3 (Right)
				...
				screen.render( xo + 8 + 4, yo + 8, 7 + 13 * 32, Color
						  .get( -1, 555, 555, 555 ), 2 ); // render a mirrored half-slash below it.
				if ( attackItem != null ) { // if the player has an item
		  ...
		  if ( attackTime > 0 && attackDir == 0 ) {  // if the attack time is larger than 0 and the attack Direction is 0 (Down)
				...
				screen.render( xo + 8, yo + 8 + 4, 6 + 13 * 32, Color
						  .get( -1, 555, 555, 555 ), 3 ); // render a mirrored half-slash to the right of it.
				if ( attackItem != null ) { // if the player has an item
	 ...
	 public int getLightRadius( ) {
		  ...
		  if ( activeItem != null ) { // if the player has an item
				if ( activeItem instanceof FurnitureItem ) { // if item is a furniture item
					 ...
					 if ( rr > r )
						  r = rr; // if the furniture's light radius is larger than the player's, then the players light radius will equal that of the furniture's.
				}
		  }
