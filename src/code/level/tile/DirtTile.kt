...
class DirtTile(id: Int): Tile(id) {//assigns the id
	...
	override fun render(screen: Screen, level: Level, x: Int, y: Int) {
		...
		screen.render(x * 16 + 0, y * 16 + 0, Sprite0x1(0, 0), col, 0) // renders the top-left part of the tile
		screen.render(x * 16 + 8, y * 16 + 0, Sprite0x1(1, 0), col, 0) // renders the top-right part of the tile
		screen.render(x * 16 + 0, y * 16 + 8, Sprite0x1(2, 0), col, 0) // renders the bottom-left part of the tile
		screen.render(x * 16 + 8, y * 16 + 8, Sprite0x1(3, 0), col, 0) // renders the bottom-right part of the tile
	}
