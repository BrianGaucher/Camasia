...
public class Mob extends Entity {
	 ...
	 public boolean move(int xa, int ya) { // Move the mob, overrides from Entity
		  if ( isSwimming( ) ) { // Check if the mob is swimming, ie. in water/lava
				if ( swimTimer++ % 2 == 0 )
					 return true; // Increments swimTimer, and continues only every second time (when swimTimer is not divisible by 2 evenly)
		  }
		  ...
		  if ( hurtTime > 0 )
				return true; // If we have been hurt recently and haven't yet cooled down, don't continue with the movement (so only knockback will be performed)
	 ...
	 public void heal(int heal) { // Restore health on the mob
		  ...
		  level.add( new TextParticle( "" + heal, x, y, Color
					 .get( -1, 50, 50, 50 ) ) ); // Add a text particle in our level at our position, that is green and displays the amount healed
	 ...
	 protected void doHurt(int damage, int attackDir) { // Actually hurt the mob, based on only damage and a direction
		  ...
		  level.add( new TextParticle( "" + damage, x, y, Color
					 .get( -1, 500, 500, 500 ) ) ); // Make a text particle at our position in our level, bright red and displaying the damage inflicted
	 ...
	 public boolean findStartPos(Level level) { // Find a place to spawn the mob
		  ...
		  if ( level.player != null ) { // If there is a player in our level
				...
				if ( xd * xd + yd * yd < 80 * 80 )
					 return false; // Use pythagoras' theorem to determine the distance between us and the player, and if it is less than 80 (too close) then return false
		  }
		  ...
		  if ( level.getEntities( xx - r, yy - r, xx + r, yy + r ).size( ) > 0 )
				return false; // Get a list of mobs in the level, within a box centered on our attempted coordinates, with dimensions of r times 2, and if there are any close to us, return false;
		  
		  if ( level.getTile( x, y )
						.mayPass( level, x, y, this ) ) { // Check if the tile we're trying to spawn on is not solid to us
