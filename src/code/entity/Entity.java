...
public class Entity {
	 ...
	 public int x;// column & row coordinates on the map
	 public int y;// column & row coordinates on the map
	 ...
	 public boolean move(int xa, int ya) {
		  if ( xa != 0 || ya != 0 ) { // If the horizontal acceleration OR vertical acceleration does NOT equal 0 then...
				...
				if ( xa != 0 && move2( xa, 0 ) )
					 stopped = false; // If the horizontal acceleration and the movement was successful then stopped equals false.
				if ( ya != 0 && move2( 0, ya ) )
				 ...
				 * These bit shift operators are used to easily get the X & Y coordinates of a tile that the entity is standing on. */
					 ...
					 // Calls the steppedOn() method in a tile's class. (like sand or lava)
		...
		/* Note: I was tired when typing this part, please excuse grammar quirks in the writing. (Or just re-write it to make it more sensible, lol) */
		  ...
		  for ( int yt = yt0; yt <= yt1; yt++ ) // cycles through yt0 to yt1
				 ...
				 * yt is smaller or equal to yto1 then, skip the rest of the code and go to the next cycle */
					 ...
					 level.getTile( xt, yt )
							.bumpedInto( level, xt, yt, this );  // Calls the bumpedInto function in a tile's class (like cactus)
					 if ( !level.getTile( xt, yt )
									.mayPass( level, xt, yt, this ) ) { // If the entity cannot pass this block...
		  ...
		  List<Entity> wasInside = level
					 .getEntities( x - xr, y - yr, x + xr, y + yr ); // gets all of the entities that are inside this entity (aka: colliding)
		  List<Entity> isInside = level
					 .getEntities( x + xa - xr, y + ya - yr, x + xa + xr, y + ya + yr ); // gets the entities that this entity will touch.
