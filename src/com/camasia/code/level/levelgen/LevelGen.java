package com.camasia.code.level.levelgen;
...
public class LevelGen {
    ...
    public static byte[][] createAndValidateUndergroundMap(int w, int h, int depth) {
        do { // Keep repeating this loop until it's done...
            ...
            if (depth < 3) if (count[Tile.stairsDown.id & 0xff] < 2)
    ...
    private static byte[][] createTopMap(int w, int h) {
        ...
        for (int i = 0; i < w * h / 2800; i++) {
            int xs = randomGenerator.nextInt(w); // A random number between 0 to the map's width (minus 1, because 0 is the first number)
            int ys = randomGenerator.nextInt(h); // A random number between 0 to the map's height (minus 1, because 0 is the first number)
            for (int k = 0; k < 10; k++) { // a loop inside the main loop that occurs 10 times.
                int x = xs + randomGenerator.nextInt(21) - 10; // x value which is: xs + (random value between 0 to 20) - 10
                int y = ys + randomGenerator.nextInt(21) - 10; // y value which is: ys + (random value between 0 to 20) - 10
                for (int j = 0; j < 100; j++) { // A loop inside a loop inside the main loop, repeats 100 times.
                    int xo = x + randomGenerator.nextInt(5) - randomGenerator.nextInt(5); // xo value: x + (random value between 0 to 4) - (random val between 0 to 4)
                    int yo = y + randomGenerator.nextInt(5) - randomGenerator.nextInt(5); // yo value: y + (random value between 0 to 4) - (random val between 0 to 4)
        ...
        for (int i = 0; i < w * h / 400; i++) {
            int x = randomGenerator.nextInt(w);// A random number between 0 to the map's width (minus 1, because 0 is the first number)
            int y = randomGenerator.nextInt(h);// A random number between 0 to the map's height (minus 1, because 0 is the first number)
            for (int j = 0; j < 200; j++) { // A loop that occurs 200 times
                int xx = x + randomGenerator.nextInt(15) - randomGenerator.nextInt(15); // x + (random value between 0 to 14) - (random value between 0 to 14)
                int yy = y + randomGenerator.nextInt(15) - randomGenerator.nextInt(15); // y + (random value between 0 to 14) - (random value between 0 to 14)
                if (xx >= 0 && yy >= 0 && xx < w && yy < h) { // if xx or yy is equal or larger than 0, and smaller than the width and height of the map...
                    if (map[xx + yy * w] == Tile.grass.id) { // If the specific xx and yy coordinates happen to be a grass tile...
        ...
        for (int i = 0; i < w * h / 400; i++) {
            ...
            for (int j = 0; j < 30; j++) { // loop that occurs 30 times
                int xx = x + randomGenerator.nextInt(5) - randomGenerator.nextInt(5); // x + (random value between 0 to 4) - (random value between 0 to 4)
                int yy = y + randomGenerator.nextInt(5) - randomGenerator.nextInt(5); // y + (random value between 0 to 4) - (random value between 0 to 4)
                if (xx >= 0 && yy >= 0 && xx < w && yy < h) { // if xx or yy is equal or larger than 0, and smaller than the width and height of the map...
                    if (map[xx + yy * w] == Tile.grass.id) { // If the specific xx and yy coordinates happen to be a grass tile...
                        map[xx + yy * w] = Tile.flower.id; // replace the tile with a flower tile
                        data[xx + yy * w] = (byte) (col + randomGenerator.nextInt(4) * 16); // Adds data to the tile, (flipping it sideways)
                    }
                }
            }
        }
        ...
        for (int i = 0; i < w * h / 100; i++) {
            int xx = randomGenerator.nextInt(w);// A random number between 0 to the map's width (minus 1, because 0 is the first number)
            int yy = randomGenerator.nextInt(h);// A random number between 0 to the map's height (minus 1, because 0 is the first number)
            if (xx >= 0 && yy >= 0 && xx < w && yy < h) { // if xx or yy is equal or larger than 0, and smaller than the width and height of the map...
        ...
        stairsLoop:
        for (int i = 0; i < w * h / 100; i++) {
            int x = randomGenerator.nextInt(w - 2) + 1; // A random number between 0 to the map's width minus 2, plus one.
            int y = randomGenerator.nextInt(h - 2) + 1; // A random number between 0 to the map's width minus 2, plus one.
    ...
    private static byte[][] createUndergroundMap(int w, int h, int depth) {
...

        {
            ...
            for (int i = 0; i < w * h / 400; i++) {
                ...
                for (int j = 0; j < 30; j++) { // A loop that occurs 30 times
                    int xx = x + randomGenerator.nextInt(5) - randomGenerator.nextInt(5); // x + (random number between 0 to 4) - (random number between 0 to 4)
                    int yy = y + randomGenerator.nextInt(5) - randomGenerator.nextInt(5); // y + (random number between 0 to 4) - (random number between 0 to 4)
        ...
        if (depth < 3) { // If the depth is smaller than 3 then...
            ...
            stairsLoop:
            for (int i = 0; i < w * h / 100; i++) {
                int x = randomGenerator.nextInt(w - 20) + 10; // A random number between 0 to the map's width minus 20, plus 10.
                int y = randomGenerator.nextInt(h - 20) + 10; // A random number between 0 to the map's width minus 20, plus 10.
